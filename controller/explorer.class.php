...
class explorer extends Controller{
    ...
    function playerSkin(){
        include($this->config['skin_setting']);
        $filetype=array(
            'music' => array('mp3','wma','wav','mid','acc'),
            'movie' => array('avi','flv','f4v','wmv','3gp','rmvb','mp4')
        );
        if (in_array($this->in['type'], $filetype['music'])) {
            $skin=$skin_media[$this->config['musictheme']];
        }
        else{
            $skin=$skin_media[$this->config['movietheme']];
        }
        echo json_encode(
            array(
                'title' => $filename.'——音乐播放',
    ...
    function zip(){
        ...
        for ($i=0; $i < $list_num; $i++) { 
            $zip_list[$i]['file'] = iconv(
                $this->config['app_charset'],
                $this->config['system_charset'],
                urldecode($zip_list[$i]['file'])
            );
        }
        ...
        $len = 25;
        $zipname_app = iconv($this->config['system_charset'],$this->config['app_charset'],$zipname);
    ...
    function image(){
        ...
        if (!file_exists($image_thum)){//如果拼装成的url不存在则没有生成过
            if ($_SESSION['this_path']==$this->config['pic_thumb']){//当前目录则不生成缩略图
                ...
                $cm->BackFill($image_thum,72,64);//等比例缩略图，空白处填充颜色
            }
        }

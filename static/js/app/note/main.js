...
Main.Config = {
	...
	treeAjaxURL:"?node/folderList&firstpath=",//树目录请求
	...
	filetype : {
		...
		'code'	: ['js','css','php','md','pl','vbs','sh','less','rb','xml',
			'c','cpp','h','cs','asp','java','asp','py','jsp','sql','bat','cmd'],
		'text'	: ['txt','ini','inf'],
...
Main.Global = {
	...
	canPast:false,				// 剪贴板是否有数据，决定是否可以粘贴
};
...
Main.UI = (function() {
	...
	var _f5 = function(is_data_server,is_animate) {
		...
		if(!is_data_server){//采用当前数据刷新,用于显示模式更换
			...
			$.ajax({
				url:'?node/pathList&path='+this_path,
				dataType:'json',
	...
	return{	
		...
		init:function(){
			...
			$.ajax({
				url:'?node/pathHasCopy',
				success:function(data){
					if (data == '1') Main.Global.canPast = true;			
				}
			});
